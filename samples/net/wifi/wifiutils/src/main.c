...
static int cmd_memtest(const struct shell *shell, size_t argc, char **argv)
{
    ...
    //if (!rpu_validate_addr(addr, num_words*4,&hl_flag)) return -1;
...
static int cmd_wifi_on(const struct shell *shell, size_t argc, char **argv)
{
	...
	ret = rpu_init();
	if (ret) {
		shell_print(shell, "%s: RPU init failed with error %d", __func__, ret);
		return -1;
	}

	ret = dev->init(qspi_defconfig());
	if (ret) {
		shell_print(shell, "%s: QSPI device init failed", __func__);
		return -1;
	}

	ret = rpu_enable();
	if (ret) {
		shell_print(shell, "%s: RPU enable failed with error %d", __func__, ret);
		return -1;
	}
	k_sleep(K_MSEC(10));
	
	shell_print(shell, "Wi-Fi ON done");
	wifi_on_flag = 1;
	return SHELL_OK;
...
static int cmd_wifi_off(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "Wi-Fi OFF done");
	wifi_on_flag = 0;
	return SHELL_OK;
}
